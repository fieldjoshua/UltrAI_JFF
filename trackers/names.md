...
### Terms
- **API_CLIENT**: Native fetch-based HTTP client for backend communication (no axios)
- **POLLING**: Real-time status checking via 2-second intervals (not fake timers)
- **HOOK**: React custom hook encapsulating API logic and state management
- **BI-DIRECTIONAL_DEPS**: CI dependency detection that tests backend when frontend API layer changes
- **REAL_TIMERS**: Actual setInterval delays in tests (not vi.useFakeTimers)

### File Names
- **frontend/src/services/api.js**: API client with GET/POST methods
- **frontend/src/hooks/useUltrAI.js**: Query submission and run tracking hook
- **frontend/src/hooks/useHealth.js**: Backend health monitoring hook
- **frontend/src/hooks/useCocktails.js**: Static cocktail configuration hook
- **frontend/vitest.config.js**: Vitest testing configuration
- **frontend/src/test/setup.js**: Test setup file
- **frontend/src/services/__tests__/api.test.js**: API client tests
- **frontend/src/hooks/__tests__/useUltrAI.test.js**: useUltrAI hook tests
- **frontend/src/hooks/__tests__/useHealth.test.js**: useHealth hook tests
- **frontend/src/hooks/__tests__/useCocktails.test.js**: useCocktails hook tests

### Data Structure Fields (API Responses)
- **run_id**: Unique run identifier returned from POST /runs
- **phase**: Current pipeline phase (e.g., '03_initial.json', '05_ultrai.json')
- **round**: Current round (R1, R2, R3) or null
- **completed**: Boolean indicating if run is finished
- **artifacts**: Array of artifact filenames generated by run
- **status**: Health check status ('ok' or other)
- **error**: Error message (null if no error)

## PR 22 — UI Components (Functional Basics)

### Terms
- **WIZARD_FLOW**: Multi-step user interface (Query → Cocktail → Confirm → Activate)
- **ORDER_RECEIPT**: Running summary panel showing selected options as user progresses
...
- **frontend/src/components/StatusDisplay.jsx**: Real-time run status display
- **frontend/src/components/ResultsDisplay.jsx**: Final synthesis results viewer
- **frontend/src/components/steps/Step1QueryInput.jsx**: Query input form (step 1)
- **frontend/src/components/steps/Step2CocktailSelector.jsx**: Cocktail selection (step 2)
- **frontend/src/components/steps/Step3Confirm.jsx**: Confirmation and activation (step 3)
- **frontend/src/App.jsx**: Main app with wizard orchestration

### Data Structure Fields (Component State)
- **currentStep**: Integer (1-3) tracking wizard progress
- **order**: Object containing query, cocktail, cocktailDescription
- **query**: User's text query
- **cocktail**: Selected cocktail ID (PREMIUM, SPEEDY, BUDGET, DEPTH, LUXE)
- **cocktailDescription**: Human-readable cocktail description

### UI Flow
1. Step 1: User enters query → transfers to receipt
2. Step 2: User selects cocktail → transfers to receipt
3. Step 3: User reviews → "Activate UltrAI" button emerges
4. Step 4: Status display (polling every 2s)
5. Step 5: Results display (synthesis complete)
